...
export default function Dashboard() {
  ...
  useEffect(() => {
    // 1. Get user from local storage (fixed key to match your signup logic)
    ...
    setConfig({
      userName: userData.email ? userData.email.split("@")[0].toUpperCase() : "OPERATIVE",
  ...
  // FIXED LOGOUT: Now wipes user from DB in dev and redirects to Home (/)
  const handleLogout = async () => {
    ...
    if (storedUser) {
      ...
      // Wipe from DB during development testing only
      try {
        await fetch(`${API_URL}/api/dev-wipe-user`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email }),
        });
      } catch (err) {
        console.warn("Wipe failed, continuing with logout.");
      }
    }

    localStorage.clear(); 
    router.replace("/"); // Redirect to landing page instead of refresh
  };
